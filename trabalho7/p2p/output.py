...
def print_trace(result: SearchResult) -> None:
    ...
    header = f"{'passo':<5} {'rodada':<6} {'tipo':<12} {'origem':<8} {'destino':<8} {'ttl':<4} recurso"
    print(header)
    print("-" * len(header))
    visible_events = [event for event in result.eventos if event.tipo != "ignorada"]
    if not visible_events:
        print("  nenhuma mensagem exibivel")
        return

    for visible_step, event in enumerate(visible_events, start=1):
        rodada = "-" if event.rodada is None else str(event.rodada)
        ttl = "-" if event.ttl is None else str(event.ttl)
        print(
            f"{visible_step:<5} {rodada:<6} {event.tipo:<12} "
